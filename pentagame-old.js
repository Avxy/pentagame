...
function getColourName(index) {
	switch(index) {
		...
		case 4 : return 'green';
		default : alert("Not a valid colour index: " + index);
	}
}
...
function drawPentaGame(context, isBoard) {
	var centerX = 250;
	var centerY = 250;
	var rs = 9;  // radius step (litle gray dots)
        var ros=8;   // radius outer rim step
	var rj = 19;  // radius junction
	var rc = 21;  // radius corner
	var rr = 204; // radius mid corner
	var rR = rr + rc; // outer radius
        //
	...
	for (i = 0; i < 5; i++) {
		...
		for (j = 0; j < 6; j++) { // connect each startpoint with 2 neighbor junctions and startpoints
                    var m1 = rc + rs + (2 * rs * j);
                    addCircle(context, spX - (cosinusfactor * m1), spY - (sinusfactor * m1), rs, isBoard, 'silver', i, ii+5, j+1);
                    addCircle(context, spX - (cosinusfactor * m1), spY + (sinusfactor * m1), rs, isBoard, 'silver', i, ij, j+1);
		}
                ...
                for (j = 0; j < numRimCirc; j++){
                    ...
                    xx=tempP[0];
                    yy=tempP[1];
                    addCircle(context, xx,yy, ros, isBoard, 'silver', rev? ik : i, rev? i : ik, (rev? numRimCirc-j : j+1));
                }
		...
		addCircle(context, spX - m2 , spY - m3, rj, isBoard, myColor, ii+5, 0, 0);
                ...
                for (j=1; j<=3; j++){ // connect junction spots
                    addCircle(context, spX - m2 , spY - m3 + rj + ((j*2-1)*rs), rs, isBoard, 'silver', ii+5, ij, j);
                }
